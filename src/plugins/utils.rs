...
pub fn is_correct_plugins_list(plugins: &Vec<String>, valid_plugins: &Vec<String>) -> Result<(), Error> {
    if plugins.is_empty() {
    ...
    match invalid_plugin_names.is_empty() {
        ...
        false => {
            ...
            let message = String::from(format!("The following plugins weren't found or not available: {}", values));
